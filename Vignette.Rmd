...
### Task Number
'task' is not within the variables to be directly analyzed, however it is important to check that there are no times that have significantly different times than the other. For example, if time 12 has many more 1s than 4s, this may signal that participants become less and less likely to purchase an app with more exposure to advertisements. While interesting, this is outside the scope of this project and we will only be galncing at the distributions to make sure that none are significantly different.
...
One common issue that can impact multivariate analyses of linear models is multicollinearity, or the correlation of predictor variables. We will create a matrix of the relationships between all predictors to check for mutlicollinearity.
...
The p-value (<i>p</i> = 0.21) is not small enough to provide evidence that the predictor variables are pairwise-dependent. This means we have evidence that our predictor variables are not multicollinear. 
...
<b>NOTE</b>: This task is inferential rather than predictive, so data will not need to undergo typical predictive modeling treatment such as splitting and training.

```{r Ordinal regression model}
ORModel <- experiment_data %>%
  #sample_n(8000) %>%
  clmm(answer ~ duration  + price + offer + outcome + rtb + social_proof +
         (1|response_id),
       data = .,
       nAGQ = 10,
      Hess = TRUE)
